...
class shopQdiscountRouteHelper {

    public static function saveTemplates($route_hash, $templates) {
        if (wa()->getEnv() == 'frontend') {
            throw new waException('Доступ запрещен');
        }
        foreach ($templates as $template_id => $template) {
            $s_template = shopQdiscountPlugin::$templates[$template_id];
    ...
    public static function getRouteTemplates($route_hash = null, $template_id = null, $read = true) {
        ...
        if ($template_id) {
            ...
            $templates = array();
            foreach (shopQdiscountPlugin::$templates as $template_id => $template) {
    ...
    protected static function getRouteTemplate($route_hash = null, $template_id, $read = true) {
        if (empty(shopQdiscountPlugin::$templates[$template_id])) {
            return false;
        }
        if ($route_hash === null) {
            $route_hash = self::getCurrentRouteHash();
        ...
        $template = shopQdiscountPlugin::$templates[$template_id];
    ...
    public static function getRouteSettings($route = null, $setting = null) {
        ...
        $routes = wa()->getPlugin('qdiscount')->getSettings('routes');
