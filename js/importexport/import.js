    ...
    qdiscount_import: {
        ...
        actionHandler: function ($el) {
            try {
                ...
                if (method.name) {
                    $.shop.trace('$.importexport.plugins.qdiscount', method);
        ...
        initForm: function () {
            this.$form.off('click', '#plugin-qdiscount-submit-import input[type=submit]').on('click', '#plugin-qdiscount-submit-import input[type=submit]', function (event) {
                try {
                    var $form = $(this);
                    $form.find(':input, :submit').attr('disabled', false);
                    $.importexport.plugins.qdiscount_import.qdiscountHandler();
                } catch (e) {
                    $('#plugin-qdiscount-transport-group').find(':input').attr('disabled', false);
                    $.shop.error('Exception: ' + e.message, e);
                }
                return false;
            });
        ...
        qdiscountHandler: function () {
            ...
            this.form.find('#plugin-qdiscount-submit-import .progressbar .progressbar-inner').css('width', '0%');
            this.form.find('#plugin-qdiscount-submit-import .progressbar').show();
            var url = '?plugin=qdiscount&module=import';
            var self = this;
            $.ajax({
                url: url,
                data: data,
                ...
                success: function (response) {
                    if (response.error) {
                        ...
                        self.form.find('#plugin-qdiscount-submit-import .js-progressbar-container').hide();
                        self.form.find('.shop-ajax-status-loading').remove();
                        self.progress = false;
                        self.form.find('.errormsg').text(response.error);
                    } else {

                        self.form.find('#plugin-qdiscount-submit-import .progressbar').attr('title', '0.00%');
                        self.form.find('#plugin-qdiscount-submit-import .progressbar-description').text('0.00%');
                        self.form.find('#plugin-qdiscount-submit-import .js-progressbar-container').show();
                ...
                error: function (response) {
                    if (response.responseText) {
                        self.form.find('#plugin-qdiscount-submit-import .errormsg').html(response.responseText);
                    } else {
                        self.form.find('#plugin-qdiscount-submit-import .errormsg').html('Ошибка: ' + response.status);
                    }
                    self.form.find(':input').attr('disabled', false);
                    self.form.find('a.js-action:hidden').each(function () {
                        var $this = $(this);
                    ...
                    self.form.find(':submit').show();
                    self.form.find('#plugin-qdiscount-submit-import .js-progressbar-container').hide();
                    self.form.find('#plugin-qdiscount-submit-import .shop-ajax-status-loading').remove();
                    self.form.find('#plugin-qdiscount-submit-import .progressbar').hide();
                }
            });
            return false;
        },
        ...
        progressHandler: function (url, processId, response) {
            ...
            var self = $.importexport.plugins.qdiscount_import;
            var $bar;
            if (response && response.ready) {
                ...
                $bar = self.form.find('#plugin-qdiscount-submit-import .progressbar .progressbar-inner');
                $bar.css({
                    'width': '100%'
                });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#plugin-qdiscount-submit-import").hide();
                        self.form.find('#plugin-qdiscount-submit-import .progressbar').hide();
                        var $report = $("#plugin-qdiscount-report-import");
                        $report.show();
                        if (response.report) {
                ...
                if (response && (typeof (response.progress) != 'undefined')) {
                    ...
                    $description = self.form.find('#plugin-qdiscount-submit-import .progressbar-description');
