$.extend($.importexport.plugins, {
    qdiscount_export: {
        ...
        initForm: function () {
            this.$form.off('click', '#plugin-qdiscount-submit-export input[type=submit]').on('click', '#plugin-qdiscount-submit-export input[type=submit]', function (event) {
                try {
                    var $form = $(this);
                    $form.find(':input, :submit').attr('disabled', false);
                    $.importexport.plugins.qdiscount_export.qdiscountHandler();
                } catch (e) {
        ...
        qdiscountHandler: function () {
            ...
            this.form.find('#plugin-qdiscount-submit-export .progressbar .progressbar-inner').css('width', '0%');
            this.form.find('#plugin-qdiscount-submit-export .progressbar').show();
            var url = '?plugin=qdiscount&module=export';
            var self = this;
            $.ajax({
                url: url,
                data: data,
                ...
                success: function (response) {
                    if (response.error) {
                        ...
                        self.form.find('#plugin-qdiscount-submit-export .js-progressbar-container').hide();
                        self.form.find('.shop-ajax-status-loading').remove();
                        self.progress = false;
                        self.form.find('.errormsg').text(response.error);
                    } else {
                        
                        self.form.find('#plugin-qdiscount-submit-export .progressbar').attr('title', '0.00%');
                        self.form.find('#plugin-qdiscount-submit-export .progressbar-description').text('0.00%');
                        self.form.find('#plugin-qdiscount-submit-export .js-progressbar-container').show();
                ...
                error: function (response) {
                    if (response.responseText) {
                        self.form.find('#plugin-qdiscount-submit-export .errormsg').html(response.responseText);
                    } else {
                        self.form.find('#plugin-qdiscount-submit-export .errormsg').html('Ошибка: ' + response.status);
                    }
                    self.form.find(':input').attr('disabled', false);
                    self.form.find('a.js-action:hidden').each(function () {
                        var $this = $(this);
                    ...
                    self.form.find(':submit').show();
                    self.form.find('#plugin-qdiscount-submit-export .js-progressbar-container').hide();
                    self.form.find('#plugin-qdiscount-submit-export .shop-ajax-status-loading').remove();
                    self.form.find('#plugin-qdiscount-submit-export .progressbar').hide();
                }
            });
            return false;
        },
        ...
        progressHandler: function (url, processId, response) {
            ...
            var self = $.importexport.plugins.qdiscount_export;
            var $bar;
            if (response && response.ready) {
                ...
                $bar = self.form.find('#plugin-qdiscount-submit-export .progressbar .progressbar-inner');
                $bar.css({
                    'width': '100%'
                });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#plugin-qdiscount-submit-export").hide();
                        self.form.find('#plugin-qdiscount-submit-export .progressbar').hide();
                        var $report = $("#plugin-qdiscount-report-export");
                        $report.show();
                        if (response.report) {
                ...
                if (response && (typeof (response.progress) != 'undefined')) {
                    ...
                    $description = self.form.find('#plugin-qdiscount-submit-export .progressbar-description');
